 ...
 *
 * 2026-09-23: the trim itself moved to `defense.ts` (`reboundTrim`, exported) so it applies once,
 * inside `computeDefensiveImpact`, upstream of the ladder AND the DARKO-correction "expected"
 * baseline both — previously this file trimmed only its own `displayDefenseRaw` read, so the box
 * impact `darkoDefenseBonus` regresses against still saw the untrimmed excess, part of the same
 * box-formula/DARKO-correction coupling the Curry investigation found the same session. See that
 * function's own docstring for the measured Wallace/Gobert-preserving, Magic/Barkley-reducing
 * comparison against fully removing rebounding instead.
 */
 ...
 * spans with zero corroborating evidence — "productive, unconfirmed" is a real and fair reading
 * of Barkley's rebounding-and-steals profile. Spans with a real DARKO bonus (Shawn Marion, Andre
 * Iguodala, Lonzo Ball) are exempt because the data does corroborate them, and pre-1969 spans
 * predate All-Defense entirely but sit far below this ceiling anyway (no steals/blocks were
 * recorded before 1973-74, so their raw is rebounding-only) — the ceiling never binds there.
 *
 * 2026-09-23, user ask ("Magic i Barkley bliżej 60, okolice C"): lowered 78 -> 58 (was originally
 * pinned at the top of the B band; now sits at the top of C+). Swept every integer 50-60 against
 * both the headline pair and the full 46-name reference suite — 58 is the first value (walking
 * down from 78) that reaches the suite's best-observed score (8/46 miss, tied with 50-57; 59+
 * regresses to 9/46 as James Harden's own ceiling breaks through his 0-58 target) while landing
 * Barkley/Magic closest to the requested ~60 (55/64 at 58, vs e.g. 48/58 at 50 — Barkley and Magic
 * pull in opposite directions as this constant moves, so no single value lands both exactly on
 * 60; minimized summed distance instead). Curry (67) and Dirk (58) both already sit clear of this
 * ceiling in that range and were unaffected by the whole sweep.
 */
const UNCORROBORATED_CEILING = 58;
...
// 2026-09-23: rung raw values regenerated (`calibrateDefensiveTalent.ts report`, rebPctile=90/
// rebRate=0.6 — same trim shape as before, now applied inside `computeDefensiveImpact` itself
// via `defense.ts`'s `reboundTrim`, not just here) after moving the rebounding trim upstream —
// see that function's own docstring for why. Points column unchanged; only the raw rungs moved,
// since the raw formula's own distribution shifted once rebounding's excess trims out earlier.
const LADDER_PERCENTILE_POINTS = [5, 20, 32, 45, 54, 66, 75, 84, 92, 99];
const LADDER_RAW_BY_POSITION: Record<Position, number[]> = {
  PG: [-6.03, 4.04, 9.94, 13.24, 16.15, 20.27, 23.61, 26.68, 29.33, 33.71],
  SG: [-6.45, 4.26, 9.59, 12.9, 15.17, 18.92, 22.59, 27.4, 31.63, 38.91],
  SF: [-5.71, 6.07, 11.81, 16.01, 18.72, 22.8, 26.25, 30.15, 34.57, 42.18],
  PF: [-4.63, 8.69, 14.0, 19.4, 22.29, 27.07, 31.15, 35.63, 41.22, 46.84],
  C: [-4.38, 11.27, 18.16, 23.89, 27.34, 33.16, 38.95, 45.54, 51.28, 60.11],
};
...
/**
 * 2026-09-23, user's own follow-up on Barkley still reading too high after the BPM2-only bonus
 * floor above: real listed height, independent of any box stat — a genuinely undersized PF/C is
 * at a structural disadvantage (rim position, contesting shots against size) box counting stats
 * don't fully capture, the same class of "the box misses this" gap `onOffDefenseFloor`'s own
 * `PHYSICAL_ANCHOR_WEIGHT_LBS` already rewards in the OTHER direction (a heavy anchor's real wall-
 * defense). Thresholds are each position's own real p25 height in this dataset (PF 80in/6'8",
 * C 82in/6'10") — Barkley (78in/6'6") sits two inches under. Applied to `credited` only, before
 * the floor `Math.max` below — a genuinely corroborated undersized big (Draymond Green, Dennis
 * Rodman: both also 78-79in) is untouched in practice because real accolades/DARKO already floor
 * them well above whatever `credited` reads; verified directly, not assumed.
 */
const UNDERSIZED_BIG_HEIGHT_THRESHOLD: Partial<Record<Position, number>> = { PF: 80, C: 82 };
...
function undersizedBigMalus(span: PlayerSpan): number {
  ...
  const shortfall = threshold - height;
