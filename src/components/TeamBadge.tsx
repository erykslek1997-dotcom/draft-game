...
/** Badge C ("chip"): two colour bars and "CODE 'YY", for player stats. */
export function TeamChip({ code, seasonEnd, label }: BadgeProps) {
  const c = teamColors(code, seasonEnd);
  return (
    <span className="team-chip" title={label ?? `${code} ${seasonEnd}`}>
      ...
      <span className="team-chip-text">{code} {seasonTag(seasonEnd)}</span>
    </span>
  );
}
